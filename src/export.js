          ...
          const postsHTML = (day.posts || [])
            .map((post) => {
              ...
              if (isPost && descripcion) {
                contentHTML = `<div style="position:relative;background:#0d1f3a;border-radius:8px;padding:12px;font-size:13px;color:#C8D8E8;line-height:1.8;white-space:pre-wrap"><strong style="color:#1E90FF;font-size:10px;text-transform:uppercase;display:block;margin-bottom:4px">Descripcion</strong><span class="desc-text">${esc(descripcion)}</span><button onclick="navigator.clipboard.writeText(this.parentElement.querySelector('.desc-text').innerText).then(()=>{this.textContent='Copiado';setTimeout(()=>this.textContent='Copiar',1500)})" style="position:absolute;top:8px;right:8px;padding:4px 8px;background:#1E90FF33;color:#1E90FF;border:1px solid #1E90FF44;border-radius:6px;cursor:pointer;font-size:10px;font-weight:600">Copiar</button></div>`;
              } else {
                if (guion) {
                  contentHTML += `<div style="position:relative;background:#1a0a2a;border-radius:8px;padding:12px;font-size:13px;color:#C8D8E8;line-height:1.8;white-space:pre-wrap;margin-bottom:8px"><strong style="color:#E91E63;font-size:10px;text-transform:uppercase;display:block;margin-bottom:4px">Guion</strong><span class="guion-text">${esc(guion)}</span><button onclick="navigator.clipboard.writeText(this.parentElement.querySelector('.guion-text').innerText).then(()=>{this.textContent='Copiado';setTimeout(()=>this.textContent='Copiar',1500)})" style="position:absolute;top:8px;right:8px;padding:4px 8px;background:#E91E6333;color:#E91E63;border:1px solid #E91E6344;border-radius:6px;cursor:pointer;font-size:10px;font-weight:600">Copiar</button></div>`;
                }
                if (descripcion) {
                  contentHTML += `<div style="position:relative;background:#0d1f3a;border-radius:8px;padding:12px;font-size:13px;color:#C8D8E8;line-height:1.8;white-space:pre-wrap;margin-bottom:8px"><strong style="color:#1E90FF;font-size:10px;text-transform:uppercase;display:block;margin-bottom:4px">Descripcion</strong><span class="desc-text">${esc(descripcion)}</span><button onclick="navigator.clipboard.writeText(this.parentElement.querySelector('.desc-text').innerText).then(()=>{this.textContent='Copiado';setTimeout(()=>this.textContent='Copiar',1500)})" style="position:absolute;top:8px;right:8px;padding:4px 8px;background:#1E90FF33;color:#1E90FF;border:1px solid #1E90FF44;border-radius:6px;cursor:pointer;font-size:10px;font-weight:600">Copiar</button></div>`;
                }
              }
              if (hashtags) {
                contentHTML += `<div style="font-size:12px;color:#F5A623;margin-bottom:6px">${esc(hashtags)}</div>`;
              }
...
<h3 style="font-size:14px;font-weight:700;margin-bottom:12px;color:#64B5F6">Patron Semanal</h3>
<div style="display:flex;flex-direction:column;gap:6px">${patternHTML}</div>
</div>` : ""}
<div style="text-align:center;padding:24px;color:#64B5F6;font-size:12px;border-top:1px solid #1E3A6B;margin-top:24px">Creado por Juancito Ads</div>
</body></html>`;
}
